...
<body>
	<center>
		...
		<h3>${member.name}<spring:message code="message.board.list.welcomeMsg"/>...
			<a href="logout.fa">Log-out</a>
		</h3>
		<!-- 검색 시작 -->
		<form action="getBoardList.fa" method="post">
		...
		<table border="1" cellpadding="0" cellspacing="0" width="700">
			...
			<c:forEach items="${faqBoardList }" var="board">
				<tr>
					<td>${board.seq }</td>
					<td align="left"><a href="getBoard.fa?seq=${board.seq }">
							${board.title }</a></td>
					<td>${board.writer }</td>
		...
		<br> <a href="./adminPage/faqAdminInsert.jsp"><spring:message code="message.board.list.link.insertBoard"/></a>
	</center>
</body>
</html>